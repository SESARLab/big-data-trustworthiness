...
class DataGenerator:
    ...
    def createKVDataSet(self, sc, dataType, records, uniqueKeys, uniqueValues, keyLength,
        ...
        if dataType == "string":
            inputRDD = inputRDD.map(lambda e: (keyfmt % e[0], valuefmt % e[1]))
...
class AggregateByKey(KVDataTest):
    def runTest(self):
        self.rdd.map(lambda e: (e[0], int(e[1]))).reduceByKey(lambda x, y: x + y, self.options.reduce_tasks).count()
...
class AggregateByKeyNaive(KVDataTest):
    def runTest(self):
        self.rdd.map(lambda e: (e[0], int(e[1]))).groupByKey(self.options.reduce_tasks).mapValues(sum).count()
...
class CountWithFilter(KVDataTest):
    def runTest(self):
        self.rdd.filter(lambda e: int(e[1]) % 2).count()
