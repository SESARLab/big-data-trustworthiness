...
df = spark.createDataFrame(
    spark.sparkContext.parallelize(data), StructType(dataSchema))
...
df = df.withColumn('normalized_bias_weight',
                   df.bias_weight * normalizing_factor)
...
df1 = df1.withColumn("explode_val_1", explode(
    df1.explode_val)).drop("explode_val")
